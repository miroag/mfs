...
def test_model_wrong_url():
    with pytest.raises(AttributeError):
        scraper = KaropkaModelScraper('very wrong url')
...
def test_forum_wrong_url():
    with pytest.raises(AttributeError):
        scraper = KaropkaForumScraper('wrong url')

def test_live_karopka_forum_only_attachemnts():
    scraper = KaropkaForumScraper('http://karopka.ru/forum/messages/forum232/topic8294/message199483/#message199483', False).scan()
